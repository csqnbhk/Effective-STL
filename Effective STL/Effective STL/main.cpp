...
int main()
{
 ...
 /***********************************   vector和string（第二章）  **************************************/
  ...
  //19.标准关联容器是基于等价的，不是相等。注意区分这两个概念的区别。
  // 对于非标准的基于散列表的关联容器，有两种常见的设计模式：1.基于相等 2.基于等价。
  

   //22.不要直接修改set或multiset中的键（可能会打乱容器的有序性。导致重新排序）
   //不能直接修改容器内的数据，只能删除某个元素再插入要修改的值。
  //s.erase(it++);s.insert(it,temp),记得要递增迭代器，保持有效性，可以插入原先删除位置
 /*
   map<int, int> m;
   pair<int, int> p(12, 45);
   m.insert(p);
   m.begin()->first = 44;//map不支持修改键的值，编译通不过
   */
	
  //23.考虑使用排序的vector替代迭代器（1.设置阶段 2.查找阶段  3.重组阶段），只有排序vector才可以。（使用一些算法）



   //25.非标准的散列容器 hash_set/multiset,hash_map/multimap
